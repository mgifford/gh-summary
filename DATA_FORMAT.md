...
Generated by `generate-usage.mjs`. Contains GitHub Actions usage, Copilot subscription
status, Codespaces, and Projects data for the configured default user.

Some fields require specific token scopes and will show `available: false` with a `reason`
when the required access is not available.
...
```json
{
  ...
  "actions": {
    ...
    "estimatedComputeMinutes": "number",
    "topWorkflows": [
      {
  ...
  "copilot": {
    ...
    "orgMetrics": {
      "available": "boolean",
      "reason": "string (when available is false)",
      "orgs": [
        {
          "org": "string",
          "date": "YYYY-MM-DD",
    ...
    "items": [
      {
        ...
        "lastUsedAt": "ISO 8601 datetime | null",
...
| Section | Required Scope | Notes |
|---------|---------------|-------|
| `actions` | `public_repo` or `repo` | Public repos work without a token |
| `copilot.subscription` | `read:user` | Individual subscription status |
| `copilot.orgMetrics` | `manage_billing:copilot` | Org admin only |
| `codespaces` | `codespace` | Lists user's codespaces |
| `projects` | `read:project` | Classic Projects only |
...
To regenerate usage data (Actions, Copilot, Codespaces, Projects):

```bash
# Requires a token with appropriate scopes (see usage.json section)
export GITHUB_TOKEN=your_token_here
